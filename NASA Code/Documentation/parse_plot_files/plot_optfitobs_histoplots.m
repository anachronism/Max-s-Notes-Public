...
for i=plots
    Y = all_workspaces(i).OptimalFitnessObserved-all_workspaces(i).FitObs.*all_workspaces(i).NewtecLck;
    Y = Y(all_workspaces(i).RecvLock==1);
    [cnts,cntrs] = hist(Y,bars);
...
xlim([0.00 1])
ylim([0 0.3])
legend('Excellent','Great','Good','Okay/Poor','Location','northeast')
xlabel('Fitness Error From Ideal')
