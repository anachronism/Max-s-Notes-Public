    ...
    for mission_num=4 %[1:5] Loop over different mission profiles/fitness functions
        ...
        if updateRecurse == 1
            ...
            for i = 1:numNN
                ...
                NN{i}.base_classifier = model;  % set the base classifier in the net struct
                NN{i}.classifiers = {};   % classifiers
                NN{i}.w = [1];             % weights 
                NN{i}.initialized = false;% set to false
                NN{i}.t = 1;              % track the time of learning
                NN{i}.classifierweights = {};               % array of classifier weights
        ...
        for i = 1:numNN_exploit
            for j = 1:6    
                model.type = 'CART';          % base classifier
                NN_exploit{j,i}.a = .5;                   % slope parameter to a sigmoid
                NN_exploit{j,i}.b = 10;                   % cutoff parameter to a sigmoid
                NN_exploit{j,i}.threshold = 10e-5;         % how small is too small for error
                NN_exploit{j,i}.mclass = 2;               % number of classes in the prediciton problem
                NN_exploit{j,i}.base_classifier = model;  % set the base classifier in the net struct
                NN_exploit{j,i}.classifiers = {};   % classifiers
                NN_exploit{j,i}.w = [];             % weights 
                NN_exploit{j,i}.initialized = false;% set to false
                NN_exploit{j,i}.t = 1;              % track the time of learning
                NN_exploit{j,i}.classifierweights = {};               % array of classifier weights
...
%             episilon_reset_lim = 4e-2;
            ...
            %%%history_size=200; % <<<<< NN training window with UNIQUE actions (and its respective performance) >>> designer parameter ANALYZE IMPACT ON PERFORMANCE!!!!!
              history_size = 200;
%            history_size = 25;
%            history_size = 75;%100;%20;
%            history_size = 100;
           initial_history_size = 200;
            ...
            last_e_a=[NaN NaN NaN NaN NaN NaN];
            ...
            while elapsed_time<512%length(time_series(3).esno_viasat)%elapsed_time<512%512
...
%                     fprintf('%d ',hist_count);%%%
                ...
                if hist_count==history_size % enables training after sliding window is full
                    ...
                    Q1 = floor(history_size*nn_parallel_train*0.9);
...
%                         err
                    ...
                    %Training NN2
                    tic
                    for n_i=1:numNN_exploit % parfor
                        for n_j=1:6
                            ind = randperm(history_size);
                            ind1 = ind(1:Q1);
                            ind2 = ind(Q1:end);
...
%                             NN_exploit{n_j,n_i}=train(net_exploit,x1_exploit,y1_exploit(n_j,:)); %training per feeature NN
                            [NN_exploit{n_j,n_i},~,~,~,~,err_exploit] = ...
