...
function  [f_measure,g_mean,recall,precision,...
 ...
 % has the 
  ...
  mt = size(data_train,1); % numnber of training examples
  ...
  if net.initialized==1
    ...
    predictions = classify_ensemble(net, data_train, labels_train);
    Et = sum((predictions~=labels_train)/mt);
