...
X_mat(:,2) = [0; 0];
%X_mat(:,3) = [0; 0];
P_mats(:,:,1) = [1000 0; 0 1000];
P_mats(:,:,2) = [1000 0; 0 1000];
%P_mats(:,:,3) = [1008 0; 0 1008];
H_mats(:,:,1) = [1 0];
H_mats(:,:,2) = [1 0];
%H_mats(:,:,3) = [3 0; 0 3];
F_mats(:,:,1) = [1 d_t; 0 1];
F_mats(:,:,2) = [1 d_t; 0 1];
%F_mats(:,:,3) = [1 d_t; 0 1];
Q_val_mats(:,:,1) = [1e-3 0; 0 1e-3];
Q_val_mats(:,:,2) = [10 1e-10; 1e-10 1e-10];
%Q_val_mats(:,:,3) = [1e-3 0; 0 1e-3];
P_mix_mats = zeros(N_X_ELEMENTS,N_X_ELEMENTS,N_KALMAN_FILTERS);
%pi_ = [0.8 0.1 0.1; 0.1 0.8 0.1; 0.1 0.1 0.8];
pi_ = [0.9 0.1; 0.1 0.9];
%mu_ = [0.33333; 0.33333; 0.33333];
mu_ = [0.5; 0.5]
Z_mat = [];
%R_mat = [0.1 0; 0 0.1];
R_mat = [0.1];
...
%         % REMOVE THIS
%         X_1 = [1; 3]; X_2 = [2; 4];
%         P_1 = [1000 0; 0 1000]; P_2 = [1004 0; 0 1004];
%         H_1 = [1 0]; H_2 = [2 0];
%         Q_1_val = [1e-3 0; 0 1e-3]; Q_2_val = [1e-3 0; 0 1e-3]; 
%         
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %imm mix
        mu_h = mu_'*pi_';
        mu_ij = (pi_'*diag(mu_))'*(diag(mu_h))^-1;
        X_mix_mat = X_mat*mu_ij;
        ...
        P_mix_mats = zeros(N_X_ELEMENTS,N_X_ELEMENTS,N_KALMAN_FILTERS);
        for iter1=1:N_KALMAN_FILTERS
            for iter2=1:N_KALMAN_FILTERS
                P_mix_mats(:,:,iter1) = P_mix_mats(:,:,iter1) + (P_mats(:,:,iter2) + (X_mix_mat(:,iter1)-X_mat(:,iter2))*(X_mix_mat(:,iter1)-X_mat(:,iter2))')*mu_ij(iter2,iter1);
            end
        end
        
        % get new measurement
        %Z_mat = [TOTAL(jji-delay_Rx); TOTAL(jji-delay_Rx-1)];
        Z_mat = [TOTAL(jji-delay_Rx)];

        % Kalman Filtering
        for iter1=1:N_KALMAN_FILTERS
            X_mat(:,iter1)=F_mats(:,:,iter1)*X_mix_mat(:,iter1);
            P_mats(:,:,iter1)=F_mats(:,:,iter1)*P_mix_mats(:,:,iter1)*F_mats(:,:,iter1)'+Q_val_mats(:,:,iter1);

            S_mats(:,:,iter1) = H_mats(:,:,iter1)*P_mats(:,:,iter1)*H_mats(:,:,iter1)'+R_mat;
            K_mats(:,:,iter1)=P_mats(:,:,iter1)*H_mats(:,:,iter1)'*(S_mats(:,:,iter1)^-1);
            %e_1=((H_1*F_1*X_new)-(H_1*X_1));
            e_mats(:,:,iter1)=Z_mat-(H_mats(:,:,iter1)*X_mat(:,iter1));
            X_mat(:,iter1)=X_mat(:,iter1)+(K_mats(:,:,iter1)*e_mats(:,:,iter1));
            P_mats(:,:,iter1)=(I-K_mats(:,:,iter1)*H_mats(:,:,iter1))*P_mats(:,:,iter1);
            %P_mats(:,:,iter1)=P_mats(:,:,iter1)-K_mats(:,:,iter1)*(P_mats(:,:,iter1)*H_mats(:,:,iter1)')';
        end
        
        %Update likelihoods
        for iter1=1:N_KALMAN_FILTERS
            L_mats(iter1,:)=(1/sqrt((2*pi)^N_Z_ELEMENTS))*(1/sqrt(det(S_mats(:,:,iter1))))*exp(-0.5*e_mats(:,:,iter1)'*(S_mats(:,:,iter1)^-1)*e_mats(:,:,iter1));
        end
        
        mu_ = 1/(mu_h*L_mats) * (diag(L_mats)*mu_h');
        X_out = X_mat*mu_;
        
        X_out_series = [X_out_series X_out];
        Z_mat_series = [Z_mat_series; Z_mat];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
%         pv2_IMM(jji-delay_Rx)=X_2(1); %prediction 1-step ahead
%         
%         
%         %Update likelihoods
%         L_1=sqrt((abs(2*pi*(H_1*P_1*H_1'+R)))\1)*exp(-0.5*e_1'*((H_1*P_1*H_1'+R)\1)*e_1);
%         L_2=sqrt((abs(2*pi*(H_2*P_2*H_2'+R)))\1)*exp(-0.5*e_2'*((H_2*P_2*H_2'+R)\1)*e_2);
%         %Update model probs
...
%         K_1=P_1*H_1'*((H_1*P_1*H_1'+R)\1);
%         e_1=((H_1*F_1*X_new)-(H_1*X_1));
%         Z_ELEMENTS = [Z_ELEMENTS H_1*F_1*X_new];
%         X_1=X_1+(K_1*e_1);
%         P_1=(I-K_1*H_1)*P_1;
%         X_VECTOR = [X_VECTOR X_1];
%         
...
textArray=[];
for i=1:length(Z_mat_series)
    textArray = [textArray num2hex(single(Z_mat_series(i))) '\n'];
end
inp_mat_file_id = fopen('z_vectors.txt','w+');
fprintf(inp_mat_file_id,textArray,'s');
fclose(inp_mat_file_id);