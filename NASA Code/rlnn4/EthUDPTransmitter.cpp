...
class EthUDPTransmitter {
public:
	EthUDPTransmitter(boost::asio::io_service& io_service,
		...
		: socket_(io_service,raw_protocol_t(PF_PACKET, SOCK_RAW)),
		  ...
		  t_(io_service_, boost::posix_time::milliseconds(txIntMilSec))
		  {
			transmitInterval_ = txIntMilSec;
			...
			t_.async_wait(boost::bind(&EthUDPTransmitter::sendFrame, this,
...
private:
	...
	long transmitInterval_;

	void sendFrame(const boost::system::error_code&)
	{
		if(continue_sending_) {
			socket_.send(boost::asio::buffer(data_));

			t_.expires_at(t_.expires_at() +
				boost::posix_time::milliseconds(transmitInterval_));
			t_.async_wait(boost::bind(&EthUDPTransmitter::sendFrame, this,
										boost::asio::placeholders::error));
		}
	}
